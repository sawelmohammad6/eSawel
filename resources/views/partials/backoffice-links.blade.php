@auth
    ...
    @elseif (auth()->user()->isDeliveryman())
        <div class="mb-4 flex flex-wrap gap-3">
            <a href="{{ route('deliveryman.dashboard') }}" class="rounded-full border border-[#ffd1e3] bg-white px-4 py-2 text-sm font-semibold text-brand-rose shadow-sm">Delivery Dashboard</a>
        </div>
    @endif
@endauth
